...
templates = Jinja2Templates(directory="week8/practice_sep/templates")
...
def index(request: Request):
    if request.session.get("SIGNIN") is True:
        return RedirectResponse(url="/member_sep", status_code=HTTP_303_SEE_OTHER)
...
async def login(request: Request, signin_username: str = Form(...), signin_password: str = Form(...)):
    ...
    if user_is_member is None:
        ...
        return RedirectResponse("/member_sep", status_code=HTTP_303_SEE_OTHER)
...
def member(request: Request):
    ...
    return templates.TemplateResponse("member_sep.html", {"request": request})
...
app.mount("/static", StaticFiles(directory="week8/practice_sep/static"), name="static")
